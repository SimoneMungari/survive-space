...
{
	WINDOW * savewin= newwin(20,20,COLS/2,LINES/2);	//Menu quasi identico al menu principale, qui abbiamo solo due scelte "si" e "no"
	savewin=initscr();
	start_color();
	cbreak();
	noecho();
	...
	while(true)
	{
		...
		if(sceltafinale==0)
		{
			Savegame s(user,player->getEnemyKilled()); //Se si sceglie di salvare, viene creato un oggetto Savegame e come parametri per il costruttore c'è il nome utente e il punteggio
			s.save();	//Invocazione metodo di Savegame per salvare
			endwin();
			break;
		}
...
void chiusura() //Chiusura di sicurezza, chiude la win e distrugge gli oggetti
{	
	endwin();
	for(int i=0;i<16;i++)
		delete enemy[i];
